     ...
     searchResult(value) {
        ...
        if (data) {
             this.hasRecords = data.length == 0 ? false : true; 
               ...
               var ids = data.map(x => x.Contact.Name);
              
             console.log('idss-->'+ids); 
             //this.lstResult = ids;
             console.log('List Result---->'+this.lstResult);
             console.log('Get Records----->'+JSON.stringify(data));
         }
        else if (error) {
...
handelSelectedRecord(event){   
     ...
     this.selectedRecord = this.lstResult.find(data => data.Contact.Name === objId);
     this.selectedName = JSON.stringify(this.selectedRecord.Contact.Name);
     const removed =  this.selectedName.replaceAll('"', '');
     this.selectedName = removed;
     console.log('this selectedRecprd==?'+JSON.stringify(this.selectedRecord));
...
lookupUpdatehandler(value){   
    ...
    var val = JSON.stringify(this.selectedRecord.Id);
